...
import { getEmbeddingFromVideo, matchEmbedding, drawDetections } from '../utils/faceApi'
...
export default function Attendance() {
  ...
  async function startSession() {
    ...
    setLoading(true)
    try {
      const { data } = await axios.post('/api/sessions', { courseCode: courseCode.trim() })
  ...
  async function runScan() {
    ...
    if (currentStudents.length === 0) {
      setLastResult({ type: 'warning', message: '⚠ No registered students found' })
      return
    }

    const liveEmbedding = await getEmbeddingFromVideo(videoRef.current)
    if (!liveEmbedding) {
      setLastResult({ type: 'info', message: 'No face detected — step closer to the camera' })
      return
    }

    const match = matchEmbedding(liveEmbedding, currentStudents, 0.5)
    if (!match) {
      setLastResult({ type: 'warning', message: '⚠ Face not recognised — not a registered student' })
      return
    }
    ...
    if (markedToday.find(s => s.id === student.id)) {
      setLastResult({ type: 'info', message: `ℹ ${student.name} already marked present` })
      return
    }
    ...
    try {
      ...
      setLastResult({ type: 'success', message: `✅ ${student.name} marked PRESENT` })
    } catch (err) {
  ...
  async function endSession() {
    stopEverything()
    try {
      await axios.patch(`/api/sessions/${sessionRef.current.id}/end`)
    } catch (_) {}
    ...
    <div>
      ...
      {!session ? (
        <div className="card mb-16" style={{ maxWidth: 480 }}>
          ...
          <p className="card-subtitle">Enter the course code then start the session</p>
        ...
        <div className="card mb-16" style={{
          ...
          flexWrap: 'wrap', gap: 16, maxWidth: 720
        }}>
          <div style={{ display: 'flex', gap: 24 }}>
            <div>
              <p style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 1 }}>Course</p>
              <p style={{ fontWeight: 700, fontSize: 18 }}>{session.courseCode}</p>
            </div>
            <div>
              <p style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 1 }}>Present</p>
              <p style={{ fontWeight: 700, fontSize: 18, color: 'var(--success)' }}>{markedToday.length}</p>
            </div>
            <div>
              <p style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 1 }}>Duration</p>
              <p style={{ fontWeight: 700, fontSize: 18 }}>{sessionDuration}m</p>
            </div>
          </div>
      ...
      <div className="grid-2" style={{ alignItems: 'start' }}>
        <div className="card">
          ...
          <div className="camera-wrap" style={{ maxWidth: '100%' }}>
            {!cameraOn && (
              <div style={{
                ...
                <p style={{ fontSize: 13 }}>Start a session to activate camera</p>
              </div>
            )}
            <video ref={videoRef} muted playsInline style={{ display: cameraOn ? 'block' : 'none' }} />
            <canvas ref={canvasRef} style={{ display: cameraOn ? 'block' : 'none' }} />
            {scanning && (
          ...
          <p style={{ marginTop: 12, fontSize: 12, color: 'var(--muted)', lineHeight: 1.6 }}>
            ...
            The system automatically detects and marks them present every 2 seconds.
          </p>
        </div>
        ...
        <div className="card">
          <div className="flex justify-between items-center mb-16">
            <div>
              ...
              <p className="card-subtitle">{markedToday.length} student(s) marked</p>
            </div>
            <span className="badge badge-success">{session ? 'Session Active' : 'No Session'}</span>
          </div>
          ...
          {markedToday.length === 0 ? (
            ...
            <div className="table-wrap">
              <table>
                <thead>
                  <tr><th>#</th><th>Name</th><th>Matric No.</th><th>Time</th></tr>
                </thead>
                <tbody>
                  {markedToday.map((s, i) => (
                    <tr key={s.id}>
